...
// Exit program cancel with opstion key
...
void getUsername(char *username) {
  while (true) {
    printf("Enter Username (above %d and below %d characters): ",
           MIN_NAME_LENGTH, MAX_NAME_LENGTH);
    fgets(username, MAX_NAME_LENGTH, stdin);
    signal(SIGINT, signalHandler);
    username[strcspn(username, "\n")] = '\0';

    if (strlen(username) >= MIN_NAME_LENGTH &&
        strlen(username) <= MAX_NAME_LENGTH) {
      break;
    } else {
...
void getName(char *name) {
  while (true) {
    printf("Enter First name (above %d and below %d characters): ",
           MIN_NAME_LENGTH, MAX_NAME_LENGTH);
...
void getLastname(char *last_name) {
  while (true) {
    printf("Enter last name (above %d and below %d characters): ",
           MIN_NAME_LENGTH, MAX_NAME_LENGTH);
    fgets(last_name, MAX_NAME_LENGTH, stdin);
    signal(SIGINT, signalHandler);
    last_name[strcspn(last_name, "\n")] = '\0';

    if (strlen(last_name) >= MIN_NAME_LENGTH &&
        strlen(last_name) <= MAX_NAME_LENGTH) {
      break;
    } else {
      printf("Invalid name length. Please try again.\n");
    }
...
void getEmail(char *email) {
  while (true) {
    printf("Enter Email (above %d and below %d characters): ", MIN_EMAIL_LENGTH,
           MAX_EMAIL_LENGTH);
...
void getPassword(char *password) {
  while (true) {
    printf("Enter Password (above %d and below %d characters): ",
...
bool isUserDataEmpty(const User *user) {
  if (user->name[0] == '\0' || user->last_name[0] == '\0' ||
      user->email[0] == '\0' || user->phone_number[0] == '\0' ||
      user->dob[0] == '\0' || user->gender == '\0') {
    return true;
  }
  return false;
}
...
void modify(User *user) {
  ...
  printf("Enter what you want to modify (N for Name, L for Last name, E for "
         "Email, H for Phone, D for Date of Birth, G for Gender, U for "
         "Username): ");
  char option;
  scanf(" %c", &option);
  getchar();
  ...
  fprintf(file,
          ...
          user->phone_number, user->dob, user->gender);
...
void createAccount(UserDatabase *db) {
  ...
  fprintf(file,
          "\n\nName: %s\nLast name: %s\nEmail: %s\nPhone number: %s\nDate of "
          "birth: %s\nGender: %c\nUsername: %s\n\n",
          user->name, user->last_name, user->email, user->phone_number,
          user->dob, user->gender, user->username);
